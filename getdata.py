...
def get_data():
    ...
    trunk_out = np.load('data_gen/data_motai_3d_mix.npy')[[0, 1, 3, 4, 5, 6, 8, 9]].reshape((4, 200, -1))
    trunk_out_test = np.load('data_gen/data_motai_3d_mix.npy')[[2, 7]].reshape((1, 200, -1))
    motai = trunk_out.transpose(0, 2, 1)
    motai_test = trunk_out_test.transpose(0, 2, 1)
    branch_in = np.concatenate((trunk_out.real[..., np.newaxis], trunk_out.imag[..., np.newaxis]), axis=-1).astype(np.float32).reshape((4, 200, -1))
    branch_in_test = np.concatenate((trunk_out_test.real[..., np.newaxis], trunk_out_test.imag[..., np.newaxis]), axis=-1).astype(np.float32).reshape((1, 200, -1))
    ...
    dcPs_s = np.load('data_gen/data_dcp23.npy').astype(np.float32)[[0, 1, 3, 4, 5, 6, 8, 9]].reshape((-1,  23))
    dcPs_s_test = np.load('data_gen/data_dcp23.npy').astype(np.float32)[[2, 7]].reshape((-1,  23))
    dkxs_s = np.load('data_gen/data_dkx86.npy').astype(np.float32)[[0, 1, 3, 4, 5, 6, 8, 9]].reshape((-1, 86))
    dkxs_s_test = np.load('data_gen/data_dkx86.npy').astype(np.float32)[[2,7]].reshape((-1, 86))
    ...
    return uum_new, uum_new_test, branch_in, branch_in_test, dcPs_s, dcPs_s_test, dkxs_s, dkxs_s_test, kzs_s, kzs_s_test, y, y_test, real_2d, kzs, motai, motai_test, coodinates_zz, coodinates_zz_test
